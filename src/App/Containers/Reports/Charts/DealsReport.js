...
import ReportStore from "../../../Containers/Reports/ReportStore";
...
function DealsReport(props) {
   ...
   const addDealCount =(item, i)=>{
            ...
            <Card type="primary" size="full" className=" bg-white">
                <form onSubmit={handleSubmit(onSubmit)}>
                    <div className="flex  content-start">
                        ...
                        <div className=" content-start md:w-1/12  relative">
    ...
    return (
        <div>    
        {renderFilter()} 
        { props.dealReportsData?.views?.length>0
            ? props.dealReportsData?.views.map((item, i) => addDealCount(item,i))
          : "no data"
        }
        { props.dealReportsData?.views?.length>0
            ? props.dealReportsData?.views.map((item, i) => addDealName(item,i))
          : "no data"
        }          
        { renderChart() }         
        </div>            
