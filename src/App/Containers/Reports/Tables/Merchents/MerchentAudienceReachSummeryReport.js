...
import AudienceReachDetailedReportForMerchent from "./AudienceReachDetailReportForMerchent";
import { FontAwesomeIcon } from "@fortawesome/react-fontawesome";
import image from "../../../../assets/images/Placeholder.png";
import { Pagination } from "../../../../Components/atoms/Pagination";


const MerchentsAudienceReachSummeryReport = (props) => {
    ...
    const onSubmit = data => {
        props.getAudienceReachSummeryReportForMerchents({
            ...data,
            userId,
            ...paginationAndFilterData,
            ...
            <Card type="primary" size="full" className=" bg-white mb-2">
                <form onSubmit={handleSubmit(onSubmit)}>
                    <div className="flex content-start">
                        ...
                        <div>
                            <div className="flex flex-row gjustify-end py-2">
                                <Button
                                    type="purpeldark"
                                    className="mb-2 mt-12 p-4"
                                >
                                    {t('common.submit')}
                                </Button>
                            </div>
                    ...
                    <div className="w-4/12">
                            <div>
                                <img
                                    ...
                                    src={item.merchant.imageUrl ? item.merchant.imageUrl : image}
                                    alt="avatar"
                                />
                          </div>
                        </div>
                        <div className="w-4/12">
                            <div> { ucFirst(get(item,'merchant.businessName') ) }</div>
                        </div>
                        <div className="w-4/12">
                    ...
                    <div className="border border-teal-500 shadow-lg modal-container bg-white w-11/12 md:max-w-md mx-auto rounded shadow-lg z-50 overflow-y-auto">
                        <div className="modal-content py-4 text-left px-6">
                            <div className="flex justify-between items-center pb-3">
                                <p className="text-2xl font-bold">{t("reports.modal2Title")}</p>
                            ...
                            {console.log("Modal  ", startDate)}
                            <AudienceReachDetailedReportForMerchent startDate={startDate} endDate={endDate} option={option} />
                        </div>
                    </div>
                </div>
            </Modal>
    ...
    return(
        <div>
            ...
            { renderHeader()}
            { renderModal()}
            { props.audienceReachSummeryReportForMerchents?.views?.map((item, key)=>renderMerchent(item, key)) }
           {props.audienceReachSummeryReportForMerchents?.totalItems>paginationAndFilterData.size && (
            <Pagination
                currentPage={props.audienceReachSummeryReportForMerchents?.page}
                totalPages={props.audienceReachSummeryReportForMerchents?.totalPages}
...
export default compose(BaseHOC)(ReportStore(MerchentsAudienceReachSummeryReport))
